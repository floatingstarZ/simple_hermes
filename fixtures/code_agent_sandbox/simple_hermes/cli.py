...
def _banner(mode: str) -> str:
    body = (
        f"{BOLD}Simple Hermes{RESET}\n"
        f"A tiny educational version of Hermes.\n"
...
def _status(agent: SimpleAgent, mode: str) -> str:
    ...
    body = (
        ...
        f"Available tools: {len(getattr(agent.tools, '_tools', {}))}\n"
...
def _format_trace(agent: SimpleAgent) -> str:
    ...
    for item in trace:
        ...
        lines.append(f"step {item.step}: {item.kind}{tool_suffix} -> {item.content[:90]}")
...
def _create_prompt_session() -> "PromptSession | None":
    ...
    if not os.isatty(0) or not os.isatty(1):
        return None
    history_path = Path.home() / ".simple_hermes" / "prompt_history.txt"
...
def _read_input(session: "PromptSession | None") -> str:
    if session is not None:
        ...
        return session.prompt([
            ("class:prompt", "simple_hermes"),
            ("class:marker", " > "),
        ]).strip()
    return input("simple_hermes > ").strip()
